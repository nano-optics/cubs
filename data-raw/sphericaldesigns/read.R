library(here)
setwd(here('data-raw/sphericaldesigns'))
...
for (ii in seq_along(lf)){
  ...
  sphericaldesigns[[ii]] <- data.frame(phi = phi, theta =  theta, weight = 1/p$N)
...
sphericaldesigns_table <- data.frame(degree = pars$t[pars$N < 1e4], N=pars$N[pars$N < 1e4])

save(sphericaldesigns,file=here('data/sphericaldesigns.rda'))
save(sphericaldesigns_table,file=here('data/sphericaldesigns_table.rda'))
